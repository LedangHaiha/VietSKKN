...
import { AutoGenerateModal } from './features/project-wizard/AutoGenerateModal';
import { MasterPromptModal } from './features/documents/MasterPromptModal';
...
export const App: React.FC = () => {
  ...
  const [showMasterPromptModal, setShowMasterPromptModal] = useState<boolean>(false);
  ...
  const handleApplyFullAutoGeneratedSections = (generatedSections: SKKNSection[]) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      <Header
        ...
        onAutoGenerateFullSKKN={() => setShowAutoGenerateModal(true)}
        onOpenMasterPromptModal={() => setShowMasterPromptModal(true)}
      />
      ...
      {showAutoGenerateModal && activeProject && (
        <AutoGenerateModal
          project={activeProject}
          onClose={() => setShowAutoGenerateModal(false)}
          onApplyGeneratedSections={handleApplyFullAutoGeneratedSections}
        />
      )}

      {showMasterPromptModal && (
        <MasterPromptModal onClose={() => setShowMasterPromptModal(false)} />
      )}
    </div>
  );
};
